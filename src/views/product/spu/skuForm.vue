<template>
  <el-form>
    ...
    <el-form-item label="SPU品牌">
      <el-select placeholder="请你输入SPU品牌" v-model="spuParams.tmId">
        <el-option :value="trade.id" :label="trade.tmName" v-for="trade in allTrademark" :key="trade.id"></el-option>
      </el-select>
    </el-form-item>
  ...
  <el-dialog v-model="dialogVisible">
    ...
    <el-form-item label="SPU销售属性">
      ...
      <el-button type="primary" :icon="Plus" style="margin-left: 10">添加销售属性</el-button>
    </el-form-item>
    <el-table style="margin-left: 100px;" border>
...
import { reqAllattrList, reqAllTrademark, reqimgList, reqOwnattrList } from '@/api/product/spu';
...
let result2: any = await reqOwnattrList(row.id);
let reuslt3: any = await reqimgList(row.id);
imgList.value = reuslt3.map((item: { imgName: any; imgUrl: any; }) => {
  return {
    name: item.imgName,
    url: item.imgUrl
  }
